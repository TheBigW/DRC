...
#define TIME_DOMAIN_CONVOLUTION_BODY(channels) G_STMT_START { \
  ...
  for (i = 0; i < num_samples; i++) { \
    ...
    off = l * channels + k; \
    kernel_off = k * kernel_channel_offset; \
    for (j = 0; j <= from_input; j++) { \
      dst[i] += src[off] * kernel[j + kernel_off]; \
      off -= channels; \
    } \
    /* j == from_input && off == (l - j) * channels + k */ \
    off += kernel_length * channels; \
    for (; j < kernel_length; j++) { \
      dst[i] += buffer[off] * kernel[j + kernel_off]; \
      off -= channels; \
    } \
  } \
  \
...
{ \
  gint channels = GST_AUDIO_FILTER_CHANNELS (self); \
  GST_WARNING("channels : %i", self->channels);\
...
gst_audio_fx_base_fir_filter_push_residue (GstAudioFXBaseFIRFilter * self)
{
  ...
  if (self->channels == 0 || rate == 0 || self->nsamples_in == 0) {
    self->buffer_fill = 0;
    g_free (self->buffer);
    self->buffer = NULL;
    return;
  ...
  outsize = outsamples * self->channels * bps;
  ...
  if (!self->fft || self->low_latency) {
    ...
    diffsamples =
        ((gint64) self->latency) - ((gint64) self->buffer_fill) / self->channels;
    if (diffsamples > 0) {
      diffsize = diffsamples * self->channels * bps;
    ...
    while (gensamples < outsamples) {
      ...
      guint8 *zeroes = g_new0 (guint8, step_insamples * self->channels * bps);
      guint8 *out = g_new (guint8, self->block_length * self->channels * bps);
    ...
    const GstAudioInfo * info)
{
  ...
  gst_audio_fx_base_fir_filter_select_process_function (self,
      GST_AUDIO_INFO_FORMAT (info), self->channels);
  g_mutex_unlock (&self->lock);
...
void
gst_audio_fx_base_fir_filter_set_multi_kernel (GstAudioFXBaseFIRFilter *self, gdouble *kernel,
                                              guint kernel_length, guint64 latency, const GstAudioInfo * info, guint kernel_channels)
{
	gint channels;
	...
	gst_audio_fx_base_fir_filter_set_kernel(self, kernel, kernel_length, latency, info);
}
