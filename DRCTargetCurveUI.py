...
def writeTargetCurveFile(targetCurveFile, data):
    ...
    to 0, the last line must have a frequency equal to BCSampleRate / 2 . A
    post
...
class AddDialog(Gtk.Dialog):
    ...
    def __init__(self, parent, params=None):
        ...
        self.freqLE = LabeledEdit(box, "frequency", str(self.params[0][0]),
                                  "Frequency of the EQBand")
        self.gainLE = LabeledEdit(box, "Gain", str(self.params[0][1]),
                                  "Gain of the EQBand")
        self.show_all()

    def on_ok(self, param):
        self.params = [(float(self.freqLE.entry.get_text()),
                        float(self.gainLE.entry.get_text()))]
...
class EQControl():
    ...
    def rebuild_eq_controls(self, params):
        ...
        for i in range(0, numEqBands):
            self.eqBox.add(
                EQGroupControl(params[i][0], float(params[i][1]), self))
        self.eqBox.show_all()
    ...
    def on_remove_band(self, eqbandCtrl):
        ...
        for i in range(0, numParams):
            print("checking : " + str(eqbandCtrl.frequency) + " for : " + str(
                params[i]))
